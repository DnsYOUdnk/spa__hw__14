...
function Main () {
    ...
    this.render = (hash) => {
        ...
        if(slugOfHash == 'cart') {
            ...
            const { content, title, notFinishGif } = componentData[0];

            pageTitle.innerHTML= title;

            this.main.innerHTML= !slugOfHash.includes('/') ? `<div class="container">
                                    <h1>${title}</h1>
                                    <p>${content}</p>
                                    ${notFinishGif ? '<div class="main__unfinish__gif"><img src=' + notFinishGif + '></div>' : ""}
                                </div>` : '';
